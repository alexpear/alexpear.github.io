...
/**
 Opts
 Look for optimal shortcuts first
 . letters from destination added into current word
 . but only good sometimes
 Trace both directions at once towards middle
 Some sort of dynamic programming for a graph. 
 . appealing but vague 
 . evaluate based on length towards good letters 
 Djikstra's... mostly sounds great
 . map from word to best distance (from beginWord)
 . also need to track the unvisited set. keys in a obj, delete key?
   . no, priority queue... heap...
   . Can i track this set more simply?
   . could start with sorted array, then upgrade to heap for speed. 

 Obstacles
 When contemplating a word W, how to quickly iterate over its neighbors? 
 Sort, then build a graph?
 I want to index words on a 10-D array, maybe
 i could have obj keyed by gapwords, like lo_, _ot, with arrays as values. These are planes, for the 3-length examples. 
 */
var ladderLength = function(beginWord, endWord, wordList) {
    ...
    for (const word of wordList) {
        for (let i = 0; i < word.length; i++) {
            ...
            // LATER could support deletion with calls like add2Graph('dog', '_dog') etc
        }
    }

    // Djikstra's
    // LATER could upgrade this array to a heap
    const shortests = {};
    let unvisited = [];
    for (const word of wordList) {
        // unvisited.push({
        ...
        shortests[word] = Number.MAX_SAFE_INTEGER;
    }

    // beginWord might not be in wordList
    if (! shortests[beginWord]) {
        unvisited.push(beginWord);
    }

    shortests[beginWord] = 1;
    ...
    while (unvisited.length > 0) {
        ...
        unvisited.sort(
            (a,b) => shortests[a] - shortests[b]
        );

        const currentWord = unvisited.shift();

        for (let i = 0; i < currentWord.length; i++) {
            const gapped = currentWord.slice(0, i) +
                '_' +
                currentWord.slice(i + 1);

            const neighbors = gapGraph[gapped];
            ...
            for (const neighborWord of neighbors) {
                if (neighborWord === currentWord) {
                    continue;
                }

                const currentPathLength = shortests[currentWord] + 1;

                if (shortests[neighborWord] > currentPathLength) {
                    if (neighborWord === endWord) {
                        return currentPathLength;
                    }

                    shortests[neighborWord] = currentPathLength;
                }
            }
        }
    ...
    function add2Graph (word, gapped) {
        const value = gapGraph[gapped];

        if (! value) {
            gapGraph[gapped] = [word];
        }
        else {
            value.push(word);
        }
...
function runTests () {
    ...
    for (let test of TESTS) {
        console.log();
        console.log(test.inputs.join('\n'));

        const myOutput = ladderLength.apply(this, test.inputs);
        ...
        console.log(`(${passed ? 'PASS' : 'FAIL'}) My output was: ${myOutput}${shouldBe}`);
    }
